...
export const generatePageContent = async (query: string): Promise<WebPageContent | string> => {
  const prompt = `Topic: "${query}". Generate a concise, high-quality web page content as JSON object with exactly these fields: {"title": "Page Title", "contentHtml": "<html content here>"}. Limit content to ~500 words. Return ONLY valid JSON, no markdown, no code blocks.`;
  ...
  try {
    ...
    if (responseText.trim()) {
      try {
        const result = extractAndParseJson<WebPageContent>(responseText);
        if (result && result.title && result.contentHtml) {
          return result;
        }
      } catch (parseError) {
        console.warn("Failed to parse AI response, attempting cleanup...", parseError);
        // Try to extract and clean the response
        const cleaned = responseText.replace(/```json|```/g, '').trim();
        try {
          const cleanResult = JSON.parse(cleaned) as WebPageContent;
          if (cleanResult && cleanResult.title && cleanResult.contentHtml) {
            return cleanResult;
          }
        } catch {
          console.warn("Cleanup failed, generating synthetic content");
        }
      }
    }
    throw new Error("Invalid response format from API");
  } catch (error) {
    console.error("API Error - generating synthetic content:", error);
    
    // Generate synthetic content for any query when API fails
    const title = query.charAt(0).toUpperCase() + query.slice(1);
    const contentHtml = `
      <h1>${title}</h1>
      <p><strong>Overview:</strong> ${title} is an important topic that encompasses various aspects and applications in modern society.</p>
      
      <h2>Key Concepts</h2>
      <p>${title} involves several fundamental principles and practices that are essential to understand. These concepts build upon established knowledge and continue to evolve with new discoveries and innovations.</p>
      
      <h2>Practical Applications</h2>
      <p>In real-world scenarios, ${title} plays a significant role in various industries and everyday life. Understanding its applications helps individuals and organizations make informed decisions and implement best practices.</p>
      
      <h2>Future Perspectives</h2>
      <p>The field of ${title} continues to develop and adapt to changing circumstances. Emerging trends and technologies are shaping how we approach and implement strategies related to this topic.</p>
      
      <p><em>Note: This content was generated as a demonstration. For more detailed information, please try searching again or explore other topics.</em></p>
    `;
